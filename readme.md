...
思路&流程：
1. 截图：拍下当前屏幕画面
2. AI决策：把截图发给AI，让它决定下一步做什么
3. 解析动作：把AI返回的指令(如click(100,200))解析成可执行的代码
4. 执行操作：真正去点击鼠标、输入文字
5. 检查完成：判断任务是否完成，未完成则回到第1步。
> 检查完成有两个判断条件：模型是否输出"finished"指令、是否达到最大执行步数（递归100步）。任一条件达到则停止执行。
